...
```{r plot-expression}
plotExpression(example_sceset, rownames(example_sceset)[1:6],
               x = "Mutation_Status", exprs_values = "counts", log = TRUE)
```
...
plotExpression(example_sceset, rownames(example_sceset)[7:12],
               x = "Mutation_Status", exprs_values = "exprs", colour = "Cell_Cycle",
...
total_features vs total_counts plot above, we can change it:
...
```{r plot-pca-4comp-sizeby-exprs, fig.height=5.5}
plotPCA(example_sceset, size_by = "Gene_1000")
```

```{r plot-pca-4comp-colby-exprs, fig.height=5.5}
...
We can also use t-SNE to visualise cells in a reduced-dimension space:

```{r plot-tsne-colby-shapeby, fig.height=5.5}
plotTSNE(example_sceset, colour_by = "Treatment", shape_by = "Mutation_Status")
```

```{r plot-tsne-colby-sizeby-exprs, fig.height=5.5}
plotTSNE(example_sceset, colour_by = "Gene_0001", size_by = "Gene_1000")
```

```{r plot-tsne-4comp-sizeby-exprs, fig.height=5.5}
plotTSNE(example_sceset, size_by = "Gene_1000")
```

```{r plot-tsne-4comp-colby-exprs, fig.height=5.5}
plotTSNE(example_sceset, colour_by = "Gene_1000")
```
